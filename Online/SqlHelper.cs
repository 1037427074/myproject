...
public abstract class SqlHelper
{
    ...
    public static readonly string ConnectionStringLocalTransaction = "DATA SOURCE=localhost:1521/ORCL;PASSWORD=123456;PERSIST SECURITY INFO=True;USER ID=SA";
    ...
    {
        using (OracleConnection con = new OracleConnection(ConnectionStringLocalTransaction))
        {
            con.Open();
    ...
    public static OracleDataReader ExcuteReader(string sql, OracleParameter[] param)
    {
        OracleConnection con = new OracleConnection(ConnectionStringLocalTransaction);
        con.Open();
        OracleCommand cmd = new OracleCommand(sql, con);
    ...
    public static OracleDataReader ExcuteReader(string sql)
    {
        OracleConnection con = new OracleConnection(ConnectionStringLocalTransaction);
        con.Open();
        OracleCommand cmd = new OracleCommand(sql, con);
    ...
    public static int ExcuteNonQuery(string sql, OracleParameter[] param)
    {
        using (OracleConnection con = new OracleConnection(ConnectionStringLocalTransaction))
        {
            con.Open();
    ...
    /// <summary>
    /// 执行ExcuteAdatper来获得要查询的表
    /// </summary>
    /// <param name="sqlstr">sql语句</param>
    /// <returns>相应的表</returns>
    public static DataTable ExcuteAdatper(string sqlstr)
    {
    ...
    /// <summary>
    /// ExcuteAdatper重载，有条件的查询表
    /// </summary>
    /// <param name="sqlstr"></param>
    /// <returns></returns>
    public static DataTable ExcuteAdatper(string sqlstr, OracleParameter[] para)
    {
        DataTable dt = new DataTable();
        using (OracleConnection con = new OracleConnection(ConnectionStringLocalTransaction))
        {
    ...
    /// <param name="constr">查询条件</param>
    /// <returns>返回单元格的数据</returns>
    public static object getSingle(string table, string column, string constr)
    {
        string sqlstr = "select " + column + " from " + table + " " + constr;
        DataTable dt = ExcuteAdatper(sqlstr);
        if (dt.Rows.Count > 0)
            return dt.Rows[0][0];
        else
            return null;
